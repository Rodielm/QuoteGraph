...
import type { ReflectedQuote, Reflection } from "@/shared/types/reflection";
...
export async function fetchMyReflections(): Promise<ReflectedQuote[]> {
  const { data } = await api.get<ReflectedQuote[]>("/users/me/reflections");
  return data;
}
